...
- 使用内置 `image_gen` 生成新的源 atlas，不走 CLI fallback。
...
- 更新 `tools/build_imagegen_assets.py`，让它从新的 imagegen 源图裁剪、抠 alpha、归一化、打包到 `public/assets/...`。
...
- 由于当前处于 Plan Mode，本回复只给出可执行计划；实际 imagegen 调用和文件覆盖应在切回执行模式后进行。
